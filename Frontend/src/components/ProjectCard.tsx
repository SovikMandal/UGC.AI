...
const ProjectCard = ({
  ...
  const togglePublished = async (projectId: string) => {
    console.log("toggling published status for generation with id:", projectId);
  }
        ...
        <div className="p-4">
            <div className="flex items-center justify-between gap-4">
                <div className="flex-1">
                    ...
                    {gen.updatedAt && <p className="text-xs text-gray-500 mt-1">Updated: {new Date(gen.updatedAt).toLocaleDateString()}</p>}
                </div>
            ...
            {!forCommunity && (
                <div className="mt-4 grid grid-cols-2 gap-3">
                    <GhostButton onClick={() => {navigate(`/result/${gen.id}`); scrollTo(0,0)}} className="text-xs justify-center">
                        View Details
                    </GhostButton>
