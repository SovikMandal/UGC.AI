...
export const createProject = async (req: Request, res: Response) => {
  ...
  if (images.length < 2 || !productName) {
    return res
      .status(400)
      .json({ message: "Please Uploaded at least 2 images" });
  }
  ...
  try {
    ...
    const genrationConfig: GenerateContentConfig = {
...
export const createVideo = async (req: Request, res: Response) => {
  ...
  try {
    ...
    if(project.generatedVideo) {
      return res.status(404).json({message: "Video Already Genrated"});
    }
    ...
    if(!project.generatedImage) {
      throw new Error('Generated Imgae is not found');
    }

    const image = await axios.get(project.generatedImage, {responseType: 'arraybuffer'});

    const imageBytes: any = Buffer.from(image.data);
    let opteration: any = await ai.models.generateVideos({
      model,
      prompt,
      image: {
    ...
    while(!opteration.done) {
      console.log('Waiting for video generations...');
      await new Promise((resolve) => setTimeout(resolve, 10000));
      opteration = await ai.operations.getVideosOperation({
    ...
    if(!opteration.response.generateVideos) {
      throw new Error(opteration.response.raiMediaFilteredReasons[0])
    }

    await ai.files.download({
      file: opteration.response.generateVideos[0].video,
    ...
    res.json({message: 'Video Generation is Completed', videoUrl: uploadResult.secure_url})
